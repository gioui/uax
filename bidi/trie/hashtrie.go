...
func NewTinyHashTrie(size uint8, catcnt int8) (*TinyHashTrie, error) {
	...
	if catcnt > 50 {
		tracing.Errorf("number of categories to store may not exceed 50")
	...
	tracing.Infof("hash trie size = %d for %d categories", trie.size, trie.catcnt-1)
...
func (trie *TinyHashTrie) advanceToChild(p pointer, c category, n int) pointer {
	if trie.link[p] == 0 {
		...
		tracing.Debugf("link[%d] is unassigned, inserting first child=%d", p, c)
...
func (trie *TinyHashTrie) insertFirstbornChildAndProgress(p pointer, c category, n int) pointer {
	...
	for ; trys < tolerance; trys++ {
		...
		if trie.ch[h] == empty && trie.ch[h+pointer(c)] == empty {
			tracing.Debugf("found an empty child slot=%d→%d", h, h+(pointer(c)))
			break
		}
	}
	if trys == tolerance {
		tracing.Errorf("abort find")
		panic("abort find")
	}
...
func (trie *TinyHashTrie) insertChildIntoFamily(p, q pointer, c category) pointer {
	...
	trie.ch[q] = c
	trie.link[q] = 0
	tracing.Debugf("Inserted %d at q=%d with header=%d", c, q, trie.link[p])
	return q
}

func (trie *TinyHashTrie) moveFamily(p pointer, c category, n int) (pointer, pointer) {
	tracing.Debugf("have to move family for c=%d", c)
	//
	var h pointer                               // trial header location
	var x = pointer(n) * trie.alpha % trie.span // nominal position of header #n
	h = x + pointer(trie.catcnt) + 1            // now catcnt < h ≤ (trie.size+catcnt)
	...
	for ; trys < tolerance; trys++ {
		...
		tracing.Debugf("trying to find a home for family")
		if h < pointer(trie.size-trie.catcnt) {
			h++
		} else {
			h = pointer(trie.catcnt) + 1
		...
		tracing.Debugf("found a potential home h=%d", h)
		r = trie.link[p]
		delta = h - r
...
func (trie *TinyHashTrie) Stats() {
	...
	tracing.Infof("Trie Statistics:")
	tracing.Infof("  Size of trie:   %d", trie.size)
	tracing.Infof("  Category count: %d", trie.catcnt)
	tracing.Infof("  Links:    %d of %d (%.1f%%)", filllink, trie.size, float32(filllink)/float32(trie.size)*100)
	tracing.Infof("  Children: %d of %d (%.1f%%)", fillch, trie.size, float32(fillch)/float32(trie.size)*100)
	var memory uint64
	memory = uint64(unsafe.Sizeof(*trie))
	test := pointer(1)
	word := int(unsafe.Sizeof(test))
	memory += uint64(trie.size * 2 * word)
	tracing.Infof("  Memory:   %d bytes", memory)
}
